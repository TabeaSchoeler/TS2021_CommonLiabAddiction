...
commonLiability = GWASout[,c("CHR","BP","SNP", "A1", "A2", "est", "SE", "Pval_Estimate", "Q_chisq", "Q_chisq_df", "Q_chisq_pval", "MAF")]
listUserGWAIn=list(commonLiability)
...
names(listUserGWA) <- c("commonLiability")
str(listUserGWA)
...
saveRDS(listUserGWA, paste0(HOME,"/output/rds/",modelName, "_commonFacSumstats.rds"))
...
# Select only substance use pheotypes
...
for ( i in 1:length(GWA_original_all) ) {
  a=subset(listUserGWA[["commonLiability"]], select=c("CHR","BP","SNP", "Q_chisq", "Q_chisq_df", "Q_chisq_pval", "MAF")) # Select SNP and SNP info from one GenSEM GWA file for merging
...
MergedGWA_clean=lapply(MergedGWA, subset, select=c("CHR","BP","SNP", "A1", "A2", "BETA", "SE", "P", "Q_chisq", "Q_chisq_df", "Q_chisq_pval", "MAF", "N", "Neff")) # Reorder so the order of the columns is the same as in GenSEM output
